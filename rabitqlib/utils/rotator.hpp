...
static inline void flip_sign(const uint8_t* flip, float* data, size_t dim) {
    ...
    for (size_t i = 0; i < dim; i += kFloatsPerChunk) {
        ...
        const simde__mmask16 mask0 = simde_cvtu32_mask16(static_cast<uint32_t>(mask_bits & 0xFFFF));
        const simde__mmask16 mask1 =
            simde_cvtu32_mask16(static_cast<uint32_t>((mask_bits >> 16) & 0xFFFF));
        const simde__mmask16 mask2 =
            simde_cvtu32_mask16(static_cast<uint32_t>((mask_bits >> 32) & 0xFFFF));
        const simde__mmask16 mask3 =
            simde_cvtu32_mask16(static_cast<uint32_t>((mask_bits >> 48) & 0xFFFF));

        // Prepare sign-flip constant
        const simde__m512 sign_flip = simde_mm512_castsi512_ps(simde_mm512_set1_epi32(0x80000000));

        // Process 16 floats at a time with each mask segment
        simde__m512 vec0 = simde_mm512_loadu_ps(&data[i]);
        vec0 = simde_mm512_mask_xor_ps(vec0, mask0, vec0, sign_flip);
        simde_mm512_storeu_ps(&data[i], vec0);

        simde__m512 vec1 = simde_mm512_loadu_ps(&data[i + 16]);
        vec1 = simde_mm512_mask_xor_ps(vec1, mask1, vec1, sign_flip);
        simde_mm512_storeu_ps(&data[i + 16], vec1);

        simde__m512 vec2 = simde_mm512_loadu_ps(&data[i + 32]);
        vec2 = simde_mm512_mask_xor_ps(vec2, mask2, vec2, sign_flip);
        simde_mm512_storeu_ps(&data[i + 32], vec2);

        simde__m512 vec3 = simde_mm512_loadu_ps(&data[i + 48]);
        vec3 = simde_mm512_mask_xor_ps(vec3, mask3, vec3, sign_flip);
        simde_mm512_storeu_ps(&data[i + 48], vec3);
    }
}
   ...
   public:
    ...
    static void kacs_walk(float* data, size_t len) {
        ...
        for (size_t i = 0; i < len / 2; i += 16) {
            simde__m512 x = simde_mm512_loadu_ps(&data[i]);
            simde__m512 y = simde_mm512_loadu_ps(&data[i + (len / 2)]);

            simde__m512 new_x = simde_mm512_add_ps(x, y);
            simde__m512 new_y = simde_mm512_sub_ps(x, y);

            simde_mm512_storeu_ps(&data[i], new_x);
            simde_mm512_storeu_ps(&data[i + (len / 2)], new_y);
        }
    }
