...
struct HnswAvx512PopcntKernel {
    static inline float warmup_ip_x0_q_512(
        ...
        return hnsw_warmup_ip_x0_q_512_avx512(data, query, delta, vl, padded_dim, b_query);
    }
...
maxheap<std::pair<float, PID>> search_knn_avx512_popcnt(
    ...
    return index.search_knn_direct<HnswAvx512PopcntKernel>(rotated_query, topk);
}
