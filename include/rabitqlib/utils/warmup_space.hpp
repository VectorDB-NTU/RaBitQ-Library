...
inline float warmup_ip_x0_q(
    ...
    const uint64_t*
        query,  // pointer to query words (each 64 bits), arranged so that for
    ...
    __m512i ip_vec =
        _mm512_setzero_si512();  // will accumulate weighted popcount
                                 // intersections per block
    __m512i ppc_vec =
        _mm512_setzero_si512();  // will accumulate popcounts of data blocks

    // Loop over blocks in batches of 8
    for (size_t i = 0; i < vec_end; i += vec_width) {
        // Load eight 64-bit data blocks into x_vec.
        __m512i x_vec =
            _mm512_loadu_si512(reinterpret_cast<const __m512i*>(data + i));

        // Compute popcount for each 64-bit block in x_vec using the AVX512
        // VPOPCNTDQ instruction. (Ensure you compile with the proper flags for
        // VPOPCNTDQ.)
        ...
        // Process each query component (b_query is a compile-time constant, and
        // is small).
        for (uint32_t j = 0; j < b_query; j++) {
            // We need to gather from query array the j-th query for each of the
            // eight blocks. For block (i + k) the index is: ( (i + k) * b_query
            // + j ). We construct an index vector of eight 64-bit indices.
            ...
            // Gather 8 query words with a scale of 8 (since query is an array
            // of 64-bit integers).
            ...
            // Multiply by the weighting factor (1 << j) for this query
            // position.
    ...
    for (size_t i = 0; i < vec_end; i += 4) {
        ...
        for (uint32_t j = 0; j < b_query; j++) {
            ...
            __m256i index_vec = _mm256_setr_epi64x(base_idx, base_idx + b_query,
                                                   base_idx + 2 * b_query,
                                                   base_idx + 3 * b_query);

            // Gather query data: query[idx]
            __m256i q_vec =
                _mm256_i64gather_epi64((const long long*)query, index_vec, 8);
...
inline float warmup_ip_x0_q(const uint64_t* data, const uint64_t* query,
                            float delta, float vl,
