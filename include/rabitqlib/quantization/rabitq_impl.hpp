...
/**
 * @brief The one_bit_code function maps a data vector to a binary code by
 * recording the sign of every coordinate in an int array.
 *
 ...
 * @param binary_code Output binary code where each element is 0 or 1 based on
 * whether the corresponding residual element (data - centroid) is positive
 *
 * @return RowMajorArray<T> The residual vector (data - centroid)
 *
 * This function computes a binary code by recording the sign of every
 * coordinate in the residual vector (data - centroid). It returns the residual
 * vector for further processing.
 */
template <typename T>
inline RowMajorArray<T> one_bit_code(const T* data, const T* centroid,
                                     size_t dim, int* binary_code) {
...
/**
 * @brief The one_bit_code_with_factor function maps a data vector to a binary
 * code and computes factors for distance estimation.
 *
 * @param data Input data vector to be quantized
 * @param centroid Input center vector used as reference for quantization
 * @param dim Dimensionality of the vectors
 * @param binary_code Output binary code where each element is 0 or 1 based on
 * whether the corresponding residual element (data - centroid) is positive
 * @param f_add Output factor used in distance estimation to add to the final
 * result
 ...
 *
 * This function computes a binary code by recording the sign of every
 * coordinate in the residual vector (data - centroid) and calculates the
 * necessary factors for accurate distance estimation. The factors are used in
 * the estimator to approximate distances between the original vectors.
 */
template <typename T>
inline void one_bit_code_with_factor(const T* data, const T* centroid,
                                     size_t dim, int* binary_code, T& f_add,
                                     T& f_rescale, T& f_error,
                                     MetricType metric_type = METRIC_L2) {
    // quantize
    RowMajorArray<T> residual_arr =
        one_bit_code(data, centroid, dim, binary_code);

    // xu_cb = x_u + cb, xu_cb has same direction and different length with
    // x_bar
    float cb = -((1 << 1) - 1) / 2.F;
    RowMajorArrayMap<int> x_u(binary_code, 1, static_cast<long>(dim));
    ...
    // sqrt((1 - <o, o_bar>^2) / <o, o_bar>^2) / sqrt(dim - 1) = 3rd item in
    // following expression
    T tmp_error = l2_norm * kConstEpsilon *
                  std::sqrt((((l2_sqr * l2norm_sqr<T>(xu_cb.data(), dim)) /
                              (ip_resi_xucb * ip_resi_xucb)) -
                             1) /
                            (dim - 1));

    // 3 factors used for distance estimation, please refer to document for more
    // info. For f_rescale and 2nd item of f_add, we use the dot product of raw
    // residual (rather than the normalized one) as the denominator, thus we
    // need to multiply another l2norm. For (ip_cent_xucb / ip_resi_xucb), the
    // norm of xucb does not matter since it is also in numerator.
...
/**
 * @brief The one_bit_compact_code function maps a data vector to a compact
 * binary code and computes factors for distance estimation.
 *
 * @param data Input data vector to be quantized
 * @param centroid Input center vector used as reference for quantization
 * @param padded_dim Dimensionality of the vectors (padded to a multiple of the
 * bit-packing size)
 * @param compact_code Output compact binary code where the binary values are
 * packed into integers in big-endian order
 * @param f_add Output factor used in distance estimation to add to the final
 * result
 * @param f_rescale Output scaling factor used in distance estimation
 * @param f_error Output error bound for the distance estimation
 * @param metric_type Type of distance metric to use (L2 or inner product)
 *
 * This function computes a compact binary code by recording the sign of every
 * coordinate in the residual vector (data - centroid) and packs the binary
 * values into integers for efficient storage and computation. It also
 * calculates the necessary factors for accurate distance estimation. The
 * factors are used in the estimator to approximate distances between the
 * original vectors.
 */
template <typename T, typename TC>
inline void one_bit_compact_code(const T* data, const T* centroid,
                                 size_t padded_dim, TC* compact_code, T& f_add,
                                 T& f_recale, T& f_error,
                                 MetricType metric_type = METRIC_L2) {
    // binary code
    std::vector<int> binary_code(padded_dim);

    // get binary code
    one_bit_code_with_factor(data, centroid, padded_dim, binary_code.data(),
...
inline void one_bit_compact_codes(const T* data, const T* centroid, size_t num,
                                  size_t padded_dim, TC* compact_code, T* f_add,
                                  T* f_rescale, T* f_error,
                                  MetricType metric_type = METRIC_L2) {
...
#pragma omp parallel for if (Parallel)
    for (size_t i = 0; i < num; ++i) {
        one_bit_compact_code(data + (padded_dim * i), centroid, padded_dim,
                             ...
                             f_add[i], f_rescale[i], f_error[i], metric_type);
    }
}

// ! padded_dim % 64 == 0
template <typename T, bool Parallel = false>
inline void one_bit_batch_code(const T* data, const T* centroid, size_t num,
                               size_t padded_dim, uint8_t* packed_code,
                               T* f_add, T* f_recale, T* f_error,
                               MetricType metric_type = METRIC_L2) {
    std::vector<uint8_t> compact_codes(num * padded_dim / 8);

    one_bit_compact_codes<T, uint8_t, Parallel>(data, centroid, num, padded_dim,
                                                compact_codes.data(), f_add,
                                                f_recale, f_error, metric_type);
...
inline double best_rescale_factor(const T* o_abs, size_t dim, size_t ex_bits) {
    ...
    while (!next_t.empty()) {
        ...
        if (update_o_bar < (1 << ex_bits) - 1) {
            double t_next =
                static_cast<double>(update_o_bar + 1) / o_abs[update_id];
            if (t_next < t_end) {
...
template <typename T, typename TP>
T faster_quantize_ex(const T* o_abs, TP* code, size_t dim, size_t ex_bits,
                     double t_const) {
    constexpr double kEps = 1e-5;
    double ipnorm = 0;
...
template <typename T, typename TP>
inline T ex_bits_code(const T* residual, size_t dim, size_t ex_bits,
                      TP* ex_code, double t_const = -1) {
    ...
    if (t_const > 0) {
        ipnorm_inv =
            faster_quantize_ex(abs_res.data(), ex_code, dim, ex_bits, t_const);
    } else {
...
template <typename T, typename TP>
inline void ex_bits_code_with_factor(const T* data, const T* centroid,
                                     size_t dim, size_t ex_bits, TP* ex_code,
                                     T& f_add_ex, T& f_rescale_ex,
                                     T& f_error_ex,
                                     MetricType metric_type = METRIC_L2,
                                     double t_const = -1) {
    ConstRowMajorArrayMap<T> data_arr(data, 1, dim);
    ...
    T ipnorm_inv = ex_bits_code<T, TP>(residual_arr.data(), dim, ex_bits,
                                       ex_code, t_const);
    ...
    for (size_t i = 0; i < dim; ++i) {
        total_code(0, static_cast<long>(i)) +=
            static_cast<int>(residual_arr.data()[i] >= 0) << ex_bits;
    }

    // Factors are similar to those in one_bit_code_with_factor(), please refer
    // to document for detailed info.
...
template <typename T, typename TP>
inline void ex_bits_compact_code(const T* data, const T* centroid,
                                 size_t padded_dim, size_t ex_bits,
                                 TP* compact_code, T& f_add_ex, T& f_rescale_ex,
                                 T& f_error_ex,
                                 MetricType metric_type = METRIC_L2,
                                 double t_const = -1) {
    std::vector<uint8_t> ex_code(padded_dim);

    ex_bits_code_with_factor(data, centroid, padded_dim, ex_bits,
                             ex_code.data(), f_add_ex, f_rescale_ex, f_error_ex,
                             metric_type, t_const);
...
static inline void rabitq_scalar_impl(
    const T* data, const T* centroid, size_t dim, size_t total_bits,
    TP* total_code, T& delta, T& vl, double t_const = -1,
    ScalarQuantizerType scalar_quantizer_type =
        ScalarQuantizerType::RECONSTRUCTION) {
    std::vector<int> binary_code(dim);
    size_t ex_bits = total_bits - 1;

    RowMajorArray<T> residual_arr = rabitq_impl::one_bit::one_bit_code(
        data, centroid, dim, binary_code.data());

    if (ex_bits > 0) {
        ex_bits::ex_bits_code<T, TP>(residual_arr.data(), dim, ex_bits,
                                     total_code, t_const);
    }
    ...
    float cos_similarity =
        dot_product<T>(residual_arr.data(), u_cb.data(), dim) /
        (norm_data * norm_quan);

    if (scalar_quantizer_type == ScalarQuantizerType::RECONSTRUCTION) {
        delta = norm_data / norm_quan * cos_similarity;
    } else if (scalar_quantizer_type ==
               ScalarQuantizerType::UNBIASED_ESTIMATION) {
        delta = norm_data / norm_quan / cos_similarity;
...
template <typename T, typename TP>
static inline void rabitq_full_impl(const T* data, const T* centroid,
                                    size_t dim, size_t total_bits,
                                    TP* total_code, T& f_add, T& f_rescale,
                                    T& f_error,
                                    MetricType metric_type = METRIC_L2,
                                    double t_const = -1) {
    std::vector<int> binary_code(dim);
    ...
    if (ex_bits > 0) {
        ex_bits::ex_bits_code_with_factor<T, TP>(data, centroid, dim, ex_bits,
                                                 total_code, f_add, f_rescale,
                                                 f_error, metric_type, t_const);
    }
