...
inline void packing_1bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
...
inline void packing_2bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
#if defined(__AVX512F__) || defined(__AVX2__)
    // ! require dim % 16 == 0
    for (size_t j = 0; j < dim; j += 64) {
        ...
        __m128i vec_00_to_15 =
            _mm_loadu_si128(reinterpret_cast<const __m128i*>(o_raw));
        ...
        __m128i compact = _mm_or_si128(
            ...
            _mm_or_si128(_mm_slli_epi16(vec_32_to_47, 4),
                         _mm_slli_epi16(vec_48_to_63, 6)));
...
inline void packing_3bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
#if defined(__AVX512F__) || defined(__AVX2__)
    // ! require dim % 64 == 0
    ...
    for (size_t d = 0; d < dim; d += 64) {
        ...
        __m128i compact2 =
            _mm_or_si128(_mm_or_si128(vec_00_to_15, vec_16_to_31),
...
inline void packing_4bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
// although this part only requries SSE, computing inner product for this
// orgnization requires AVX512F, similar for remaining functions
#if defined(__AVX512F__) || defined(__AVX2__)
    // ! require dim % 16 == 0
    for (size_t j = 0; j < dim; j += 16) {
...
inline void packing_5bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
#if defined(__AVX512F__) || defined(__AVX2__)
    // ! require dim % 64 == 0
    ...
    for (size_t j = 0; j < dim; j += 64) {
        ...
        _mm_storeu_si128(reinterpret_cast<__m128i*>(o_compact + 16),
                         compact4_2);
...
inline void packing_6bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
#if defined(__AVX512F__) || defined(__AVX2__)
...
inline void packing_7bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
#if defined(__AVX512F__) || defined(__AVX2__)
...
inline void packing_8bit_excode(const uint8_t* o_raw, uint8_t* o_compact,
                                size_t dim) {
    std::memcpy(o_compact, o_raw, sizeof(uint8_t) * dim);
}

/**
 * @brief Packing ex_bits code to save space. For example, two 4-bit code will
 * be stored as 1 uint8. To compute inner product with the support of SIMD, the
 * packed codes need to be stored in different patterns. For details, please
 * check the code and comments for certain number of bits.
 *
 * @param o_raw unpacked code, code for each dim is represented by uint8
 * @param o_compact compact format of code
 * @param dim   dimension of code, NOTICE: different num of bits requried
 * different dimension padding, dim should obey the corresponding requirement
 * @param ex_bits number of bits used for code
 */
inline void packing_rabitqplus_code(const uint8_t* o_raw, uint8_t* o_compact,
