...
class QuantizedGraph {
   ...
   private:
    ...
    T (*raw_dist_func_)
    (const T*, const T*, size_t);  // dist func for raw vector
    ...
    Array<char, std::vector<size_t>,
          memory::AlignedAllocator<char, 1 << 22,
                                   true>>
        data_;  // vectors + graph + quantization codes + factors
    Rotator<T>* rotator_ = nullptr;  // data rotator
    std::unique_ptr<VisitedListPool> visited_list_pool_ = nullptr;

    // Position of different data in each row (RawData + QuantizationCodes +
    // Factors + neighborIDs) Since we guarantee the degree for each vertex
    // equals degree_bound (multiple of 32), we do not need to store the degree
    // for each vertex
    ...
    void find_candidates(PID, size_t, std::vector<AnnCandidate<T>>&,
                         HashBasedBooleanSet&,
                         const std::vector<uint32_t>&) const;

    void update_qg(PID, const std::vector<AnnCandidate<T>>&);

    void update_results(buffer::SearchBuffer<T>&, HashBasedBooleanSet&,
                        const T*);

    void scan_neighbors(const BatchQuery<T>&, PID, T*, buffer::SearchBuffer<T>&,
                        HashBasedBooleanSet&, size_t) const;
   ...
   public:
    ...
    void search(const T* __restrict__ query, uint32_t knn,
                uint32_t* __restrict__ results);
};

template <typename T>
inline QuantizedGraph<T>::QuantizedGraph(size_t num, size_t dim, size_t max_deg,
    ...
    : num_points_(num),
      ...
      dim_(dim),
      padded_dim_(dim),
      raw_dist_func_((metric_type == METRIC_IP) ? dot_product_dis<T>
                                                : euclidean_sqr<T>),
...
inline void QuantizedGraph<T>::save(const char* filename) const {
    ...
    output.write(reinterpret_cast<const char*>(&rotator_type_),
                 sizeof(RotatorType));
...
inline void QuantizedGraph<T>::load(const char* filename) {
    ...
    raw_dist_func_ =
        (metric_type_ == METRIC_IP) ? dot_product_dis<T> : euclidean_sqr<T>;
    ...
    if (rotator_->size() != padded_dim_) {
        std::cerr
            << "Bad padded_dim_ for rotator in QuantizedGraph<T>.load()\n";
        exit(1);
    }
 ...
 */
template <typename T>
inline void QuantizedGraph<T>::search(const T* __restrict__ query, uint32_t k,
                                      uint32_t* __restrict__ results) {
    ...
    while (search_pool.has_next()) {
        ...
        scan_neighbors(q_obj, cur_node, est_dist.data(), search_pool, *vis,
                       this->degree_bound_);
...
template <typename T>
void QuantizedGraph<T>::scan_neighbors(const BatchQuery<T>& q_obj, PID data_id,
                                       T* est_dist,
...
inline void QuantizedGraph<T>::update_results(
    buffer::SearchBuffer<T>& result_pool, HashBasedBooleanSet& vis,
    const T* query) {
    ...
    for (auto record : data) {
        ...
        for (uint32_t i = 0; i < this->degree_bound_; ++i) {
            ...
            if (!vis.get(cur_neighbor)) {
                ...
                result_pool.insert(
                    cur_neighbor,
                    raw_dist_func_(query, get_vector(cur_neighbor), dim_));
            }
        }
        if (result_pool.is_full()) {
            break;
...
inline void QuantizedGraph<T>::initialize() {
    ...
    rotator_ = choose_rotator<float>(dim_, rotator_type_,
    ...
    this->batch_data_offset_ =
        dim_ * sizeof(T);  // pos of packed code (aligned)
    ...
    data_ = Array<char, std::vector<size_t>,
                  memory::AlignedAllocator<char, 1 << 22, true>>(
...
inline void QuantizedGraph<T>::find_candidates(
    PID cur_id, size_t search_ef, std::vector<AnnCandidate<T>>& results,
    HashBasedBooleanSet& vis, const std::vector<uint32_t>& degrees) const {
    ...
    while (tmp_pool.has_next()) {
        ...
        scan_neighbors(q_obj, cur_candi, est_dist.data(), tmp_pool, vis,
                       cur_degree);
        if (cur_candi != cur_id) {
...
inline void QuantizedGraph<T>::update_qg(
    ...
    for (size_t i = 0; i < cur_degree; i += fastscan::kBatchSize) {
        quant::quantize_qg_batch(data, rotated_centroid.data(),
                                 std::min(cur_degree - i, fastscan::kBatchSize),
                                 padded_dim_, batch_data, metric_type_);
