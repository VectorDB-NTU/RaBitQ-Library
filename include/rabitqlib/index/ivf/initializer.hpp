...
inline void parallel_for(size_t start, size_t end, size_t numThreads,
                         Function fn) {
    if (numThreads <= 0) {
            ...
            threads.push_back(std::thread([&, thread_id] {
                while (true) {
                    ...
                    try {
                        ...
                        std::unique_lock<std::mutex> last_except_lock(
                            last_except_mutex);
...
/**
 * @brief For ivf centroids, we need an intializer to get the candidate
 * clusters.
 */
class Initializer {
   protected:
   ...
   public:
    ...
    virtual void centroids_distances(
        const float*, size_t, std::vector<AnnCandidate<float>>&) const = 0;
...
class FlatInitializer : public Initializer {
   ...
   public:
    ...
    void add_vectors(const float* cent) override {
        std::memcpy(centroids_.data(), cent,
                    sizeof(float) * num_cluster_ * dim_);
    }

    void centroids_distances(
        const float* query, size_t nprobe,
...
class HNSWInitializer : public Initializer {
   ...
   public:
    explicit HNSWInitializer(size_t d, size_t k)
        : Initializer(d, k), space_(d) {
        alg_hnsw_ = new hnswlib::HierarchicalNSW<float>(&space_, num_cluster_,
                                                        M_, ef_construction_);
    }
    ...
    void add_vectors(const float* cent) override {
        ...
        parallel_for(start, rows, num_threads,
                     [&](size_t row, size_t /*thread_id*/) {
    ...
    [[nodiscard]] const float* centroid(PID id) const override {
        return reinterpret_cast<const float*>(
            alg_hnsw_->getDataByInternalId(id));
    }
