...
constexpr static std::array<int, 16> kPos = {
    3 /*0000*/, 3 /*0001*/, 2 /*0010*/, 3 /*0011*/, 1 /*0100*/, 3 /*0101*/,
    2 /*0110*/, 3 /*0111*/, 0 /*1000*/, 3 /*1001*/, 2 /*1010*/, 3 /*1011*/,
    1 /*1100*/, 3 /*1101*/, 2 /*1110*/, 3 /*1111*/,
};  // all possible combination for a 4 bit string

constexpr static std::array<int, 16> kPerm0 = {
    0,  8, 1,  9, 2,  10, 3, 11, 4,
    12, 5, 13, 6, 14, 7,  15};  // data order of packed quantization code,
                                // please refer to code and the link offered
                                // above for detailed information

template <typename T, class TA>
static inline void get_column(const T* src, size_t rows, size_t cols,
                              size_t row, size_t col, TA& dest) {
    size_t k = 0;
...
/**
 * @brief Pack quantization codes, store in blocks, the data orgnization is
 * illustrated in the link and kPerm0. Since we pack codes as 32-sized groups,
 * if the num is not a multiple of 32, we have to use some space for these
 * absent data
 *
 ...
 */
inline void pack_codes(size_t padded_dim, const uint8_t* quantization_code,
                       size_t num, uint8_t* blocks) {
    size_t num_rd =
        (num + 31) & ~31;  // round up num of vecs to multiple of batch size(32)
    ...
    for (size_t row = 0; row < num_rd; row += kBatchSize) {
        ...
        // i.e., we get the codes for 8 dims of 32 vectors and re-orgnize the
        // data layout based on the shuffle SIMD instruction used during quering
...
inline void accumulate(const uint8_t* __restrict__ codes,
    ...
    // ! here, we assume the code_length is a multiple of 64, thus the dim must
    // be a ! multiple of 16
    for (size_t i = 0; i < code_length; i += 64) {
        ...
        hi = _mm512_and_si512(_mm512_srli_epi16(c, 4),
                              lo_mask);  // code of vector 16 to 31

        res_lo = _mm512_shuffle_epi8(
            lut, lo);  // get the target value in lookup table
        res_hi = _mm512_shuffle_epi8(lut, hi);

        // since values in lookup table are represented as i8, we add them as
        // i16 to avoid overflow. Since the data order is 0, 8, 1, 9, 2, 10, 3,
        // 11, 4, 12, 5, 13, 6, 14, 7, 15, accu0 accumulates for vec 8 to 15
        // (the upper 8 bits need to be updated since they stored useless info
        // of vec 0 to 7) accu1 accumulates for vec 0 to 7 similar for accu2 and
        // accu3
    ...
    // At this point, we already have the correct accumulating result (accu0:
    // 8-15, accu1: 0-7, accu2: 16-23, accu3: 24-31), but we still need to write
    // them back to RAM. Also, each accu contains 4 lines of __m128i and we need
    // to sum them together to get the final results. 512/16=32, so we can use
    // one __m512i to contain all results. The following codes are designed for
    // this purpose. For detailed information, please check the SIMD
    // documentation.
    __m512i ret1 =
    ...
    __m256i dis0 =
        _mm256_add_epi16(_mm256_permute2f128_si256(accu0, accu1, 0x21),
    ...
    __m256i dis1 =
        _mm256_add_epi16(_mm256_permute2f128_si256(accu2, accu3, 0x21),
...
// pack lookup table for fastscan, for each 4 dim, we have 16 (2^4) different
// results ! dim % 4 == 0
template <typename T>
inline void pack_lut(size_t dim, const T* __restrict__ query,
                     T* __restrict__ lut) {
